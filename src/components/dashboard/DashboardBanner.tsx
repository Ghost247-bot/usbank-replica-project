...
const DashboardBanner = () => {
  ...
  const { data: banners, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      if (!user) {
        logger.debug('No user found for banners query');
        return [];
      }
      
      logger.debug('Fetching banners for user:', user.id);
      ...
      logger.debug('Fetched banners:', data);
      return data || [];
    },
    enabled: !!user,
  });
  ...
  // Add error logging
  if (error) {
    logger.error('Banner query error:', error);
  }

  // Show loading state briefly
  if (isLoading) {
    logger.debug('Banners loading...');
    return null;
  }

  // Don't show anything if no banners or error
  if (error || !banners || banners.length === 0) {
    logger.debug('No banners to display:', { error, banners });
    return null;
  }

  logger.debug('Rendering banners:', banners);
