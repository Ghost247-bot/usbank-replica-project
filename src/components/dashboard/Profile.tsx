...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          // If no profile found, we might need to create one or wait for AuthProvider to do it
          logger.debug('No profile found for user:', user.id);
        }
      } catch (error) {
        logger.error('Error fetching profile:', error);
      }
    ...
    const handleSave = async () => {
      ...
      setLoading(true);
      try {
        // Use user_id for upsert to ensure we hit the unique constraint
        const { error } = await supabase
          .from('profiles')
          .upsert({
            id: user.id, // Try to keep id and user_id the same for simplicity
            user_id: user.id,
            ...profileData,
